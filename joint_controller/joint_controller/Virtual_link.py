...
def update_plot():
    ...
    ax.plot([x_points[1]], [y_points[1]], [z_points[1]], 'o', color='r', markersize=8, label="Joint 1")
    ...
    ax.legend(fontsize=8, loc='center left', bbox_to_anchor=(-0.1, 0.5))
...
speeds = np.ones(RC.joint_num) * 80  # default speed (deg/s)
...
try:
    while True:
        ...
        if keyboard.is_pressed('h'):
            ...
            homing_speed = np.ones(RC.joint_num) * 20  # Slower speed for homing (deg/s)
            RC.joints_homing()  # Command homing movement.
            time.sleep(2)  # Increase delay to allow slow movement.
        ...
        if command:
            # For non-homing commands, use the default speed.
            goals = np.clip(goals, RC.servo_angle_min, RC.servo_angle_max)
            RC.joints_goto(goals, speeds)
            last_input_time = time.time()
            # Update plot every 0.2 seconds when there is input.
            if time.time() - last_plot_update_time >= 0.2:
                update_plot()
                last_plot_update_time = time.time()
            time.sleep(0.05)  # Delay to slow down input processing.
        elif time.time() - last_input_time > 0.01:
            if time.time() - last_plot_update_time >= 0.01:
                update_plot()
                last_plot_update_time = time.time()

        time.sleep(0.01)  # Increase loop sleep to slow down input processing.
